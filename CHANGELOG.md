...
聚焦**Gradle 多模块识别**与**清理视图交互优化**：Android/Gradle 多模块项目递归收集构建产物，覆盖嵌套 included build 的 `.gradle` 缓存与 `core/`、`feature/` 深层分组模块；清理详情视图移除头部清理按钮，返回入口集中到左侧。

### 新增

- **Gradle 多模块构建产物识别增强**：`android` 与 `java-gradle` 生态统一支持多模块递归收集：含 `settings.gradle` 的目录视为嵌套 included build（如 `build-logic`），额外收集其 `.gradle` 缓存目录；`core/`、`feature/` 等纯分组目录继续递归下钻查找深层模块（深度上限 3 层，跳过 `src/` 源码树与隐藏目录）；产物名称显示模块相对路径（如 `core/ui/build`），提示信息按生态区分

### 变更

- **清理详情视图交互调整**：清理详情视图下移除头部「清理」按钮（清理入口保留在底部操作栏），左侧新增「← 概览」返回入口，与「← 首页」集中排列；头部「清理」按钮的悬浮提示根据清理范围（全部 / 筛选中 / 空列表）动态显示
